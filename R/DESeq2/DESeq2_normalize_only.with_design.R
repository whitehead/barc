...
###  31 January 2022 -- Include design in DESeq2 object (if sample/color file is provided) [GB]
...
if (length(commandArgs()) < (8 - offset))
{
	...
	message("Use the colors file to get the experimental design, which is used to create the DESeq2 model.")
...
if (length(commandArgs()) > 8)
{
	message("Applying design from colors file for DESeq2 model....\n")
} else {
	message("Treating all samples as independent (so assuming no replication)....\n")
}
dds = DESeqDataSetFromMatrix(countData=counts, colData=DataFrame(samples), design=design.matrix)
