...
# version = "Version 2.2 -- 28 September 2020"	# Adapted from draw_volcano_plot_from_DESeq2_analysis.R
version = "Version 2.3 -- 2 May 2022"	# Keep gene IDs as is; change print ordering of points so labeled points are on top 
...
# y.axis.min = -10
# y.axis.max = 10
# Set max.overlaps as a high number to label all desired genes
max.overlaps = 1000
...
if ( ! is.na(genes.to.label.filename) ) {
	...
	message(paste("File with genes to label is *", genes.to.label.filename, "*", sep=""))
} else {
	...
	message("USAGE: ", this.script, " inputFile Sample.A.name Sample.B.name FDR.threshold log2FC.threshold genesFile")
...
message(paste(length(log2FC[filtered.rows.labels]), "points will be labeled, based on user input."))
...
pdf.filename = paste("Volcano_plot", input,sampleA.name, "vs",  sampleB.name, fdr.threshold, log2FC.threshold, gsub(".txt", "", basename(genes.to.label.filename)), "pdf", sep=".")
...
if (num.points.to.label < num.genes.to.label)
{
	...
	geom_text_repel(aes(log2FC[filtered.rows.labels], negLog10FDR[filtered.rows.labels]), label = gene.symbols[filtered.rows.labels], size=4, max.overlaps=max.overlaps)
} else {
	ggplot() +
	# xlim(0, 20) + 
	# ylim(y.axis.min, y.axis.max) + 
