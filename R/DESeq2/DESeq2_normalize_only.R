...
###  9 May 2024 -- round up the percentage of variation [BY]
...
{
	message("\nNormalize a matrix of raw counts with DESeq2 (and draw a PCA plot with optional group colors).")
...
# Use ERCC counts to normalize
ctrlGeneNames = grep("^ERCC-", rownames(counts), value = T)
#Is has to be a numerical or logical vector
#This creates a logical vector
ctrlGenes <- rownames(dds) %in% ctrlGeneNames
...
plotPCA_BaRC_v3 = function (x, ntop = 500)
{
	...
	{
		print(ggplot.pca(xy.12, group.color, paste("PC1:   ", round(variances[1],0), "% variance", sep=""), paste("PC2:   ", round(variances[2],0), "% variance", sep=""), ntop))
		print(ggplot.pca(xy.13, group.color, paste("PC1:   ", round(variances[1],0), "% variance", sep=""), paste("PC3:   ", round(variances[3],0),  "% variance", sep=""), ntop))
		print(ggplot.pca(xy.23, group.color, paste("PC2:   ", round(variances[2],0), "% variance", sep=""), paste("PC3:   ", round(variances[3],0),  "% variance", sep=""), ntop))
	} else {
		print(ggplot.pca.noColors(pca$x[,"PC1"], pca$x[,"PC2"], rownames(pca$x), paste("PC1:   ", round(variances[1],0),  "% variance", sep=""), paste("PC2:   ", round(variances[2],0),  "% variance", sep=""), ntop))
		print(ggplot.pca.noColors(pca$x[,"PC1"], pca$x[,"PC3"], rownames(pca$x), paste("PC1:   ", round(variances[1],0),  "% variance", sep=""), paste("PC3:   ", round(variances[3],0),  "% variance", sep=""), ntop))
		print(ggplot.pca.noColors(pca$x[,"PC2"], pca$x[,"PC3"], rownames(pca$x), paste("PC2:   ", round(variances[2],0),  "% variance", sep=""), paste("PC3:   ", round(variances[3],0),  "% variance", sep=""), ntop))
	}
