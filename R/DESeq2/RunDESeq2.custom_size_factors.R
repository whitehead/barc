...
###  Created this new code version (8 January 2022)
###		Allow/require custom size factors as input [GB]
...
if (is.na(this.script)) { this.script = "./RunDESeq2.custom_size_factors.R" }
...
if (length(commandArgs()) < (7 - offset))
{
	...
	message("  as usual EXCEPT (this code version) use custom (normalization) size factors.\n")
	message(paste("USAGE:   ", this.script, "inputCounts OutputFile group1 group2 [...] sizeFactorsFile"))
	message(paste("Example: ", this.script, "Input_DESeq2.txt DESeq2_output.txt UHR UHR brain brain customSizeFactors.txt\n"))
...
# Groups (in same order as input file) are subsequent arguments (not counting the last one)
groups = commandArgs()[(8 - offset):(length(commandArgs()) - 1)]
groups
# size factors file
sizeFactorsFile = commandArgs()[length(commandArgs())]
...
message("Reading size factors file ....")
customSizeFactors = as.numeric(read.delim(sizeFactorsFile))
...
# Print size factors
message("Original size factors")
sizeFactors(dds)

# This version only!!!
# Set size factors to custom values (so don't do estimateSizeFactor scaling)
message("\n!!! WARNING: Setting custom size factors !!!\n")
sizeFactors(dds) = customSizeFactors

# Re-create DESeq object with custom size factors
message("Updating DESeq object with custom size factors ...\n")
dds = DESeq(dds)

message("Customized size factors")
sizeFactors(dds)
