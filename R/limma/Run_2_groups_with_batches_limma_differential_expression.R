...
if (is.na(this.script)) { this.script = "./Run_2_groups_with_batches_limma_differential_expression.R" }
...
if ( ! is.na(output.file) ) {
	...
	message("Do 2-group limma analysis,")
...
input.design = read.delim(design.file, row.names=1)
design.samples = rownames(input.design)
# design.samples
# colnames(values)
...
batch = input.design[,2]

design.matrix = model.matrix(~0 + design + batch)
...
colnames(design.matrix) = gsub("batch", "", colnames(design.matrix))
...
contrast.matrix = matrix(data=NA, nrow=length(colnames(fit$coefficients)), ncol=2)
