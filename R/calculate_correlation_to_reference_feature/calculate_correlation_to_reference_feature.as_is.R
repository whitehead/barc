...
if (length(commandArgs()) < (8 - offset))
{
	...
	message("The inputCounts table is used for correlation calculations as is.\n")
	q()
}
...
if (reference.feature %in% rownames(values))
{
	values.reference = values[reference.feature,]
...
for (i in 1:nrow(values))
{
	...
	if (max(as.numeric(values[i,]), na.rm=T) != min(as.numeric(values[i,]), na.rm=T))
	{
		# this.corr.test = cor.test(as.numeric(values[i,]), as.numeric(values.reference), alternative="two.sided", method = "pearson")
		this.corr.test = cor.test(as.numeric(values[i,]), as.numeric(values.reference), alternative="two.sided", method = "pearson", use="pairwise.complete.obs")
