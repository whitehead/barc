...
# Version 1.4: 9 Feb 2023 -- Set gene name to ID for cases where a gene has no name [GB]
#
#######################################################################
...
if (! $ARGV[1])
{
	print STDERR "\nMake a gene body BED file from a GTF file,\n";
...
if ($featureType eq "transcript")
{
	...
	foreach $transcriptID (sort keys %transcript2gene)
	{
		...
		# print "$transcript2chr{$transcriptID}\t$transcript2source{$transcriptID}\tgene\t$transcript2start{$transcriptID}\t$transcript2end{$transcriptID}\t$transcript2score{$transcriptID}\t$transcript2strand{$transcriptID}\t.\tgene_id \"$transcript2gene{$transcriptID}\"; transcript_id \"$transcriptID\";\n";
	}
}
...
elsif ($featureType eq "gene")
{
	...
	while (<GTF>)
	{
		...
		if (! /^#/)
		{
			# GB -- need to reset, in case gene has no name [9 Feb 2023]
			$geneID = "";
			$transcriptID = "";
			$gene_name = "";
			...
			if ($featureDescription =~ /gene_name (\S+)/)
			{
				$gene_name = $1;
				$gene_name =~ s/\"//g;
				$gene_name =~ s/;$//g; 
			}
			# GB -- In case gene has no name, set its name to its ID [9 Feb 2023]
			else
			{
				$gene_name = $geneID;
			}
			$gene2geneSymbol{$geneID} = $gene_name;
	...
	foreach $geneID (sort keys %gene2chr)
	{
		...
		print "$gene2chr{$geneID}\t$gene2start{$geneID}\t$gene2end{$geneID}\tgene_id \"$geneID\"; gene_name \"$gene2geneSymbol{$geneID}\";\t.\t$gene2strand{$geneID}\n";
